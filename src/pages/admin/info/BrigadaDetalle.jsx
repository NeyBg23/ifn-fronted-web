import { useParams, useNavigate } from "react-router-dom"; // Hooks para obtener el id desde la ruta y para navegación
import { useEffect, useState } from "react"; // Hooks de estado y efecto
import { useAuth } from "../../../hooks/useAuth"; // Hook personalizado para obtener usuario autenticado

const BrigadaDetalle = () => {
    // Obtiene el parámetro de la ruta (idbrigada)
    const { idbrigada } = useParams();
    const navigate = useNavigate();
    
    // Estado de la brigada y sus empleados
    const [brigada, setBrigada] = useState(null);
    const [empleados, setEmpleados] = useState(null);
    const user = useAuth()

    // Obtiene información de la brigada y la guarda en estado
    useEffect(() => {
    ...
    // Obtiene empleados asignados a la brigada (usando el id)
    useEffect(() => {
        (async () => {
    ...
    // Si la brigada no ha cargado, muestra mensaje de carga
    if (!brigada) return <p>Cargando brigada...</p>;

    // Render principal del detalle
    return (
